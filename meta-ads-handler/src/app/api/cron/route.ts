...
export async function GET(req: NextRequest) {
  ...
  if (!token || !accountId || !anthropicKey) {
    return NextResponse.json({ error: 'Missing META_ACCESS_TOKEN, META_AD_ACCOUNT_ID, or ANTHROPIC_API_KEY' }, { status: 500 })
  }

  try {
    const campaigns = await fetchFullAccount(token, accountId, datePreset)
    const result = await analyzeAccount(campaigns, goals, accountType, anthropicKey)
