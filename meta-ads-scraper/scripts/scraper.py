...
Scrapes X/Twitter via Apify, scores content with Claude, saves to knowledge base JSON.
"""
...
Return ONLY valid JSON, no markdown:
{
  ...
  "applicable_to": ["ecommerce","lead_gen","brand_awareness"],
...
def score_tweet(client: anthropic.Anthropic, tweet_text: str, author: str, replies: list) -> dict | None:
    """Use Claude to score and classify a tweet."""
    reply_texts = "\n".join([
...
def main():
    ...
    for i, tweet in enumerate(unique_tweets):
        ...
        # Score with Claude
        score = score_tweet(claude, text, author, replies)
        if not score:
            skipped += 1
            continue
